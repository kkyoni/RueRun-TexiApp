			...
			<div class="ibox ">
				<div class="ibox-content">
					<div class="col-md-12 text-right">
						@php 
						$checkPermission = \App\Helpers\Helper::checkPermission(['vehicletype-create']);
						@endphp
						@if($checkPermission)
						<a class="btn btn-primary btn-sm pull-right mb-3 op-btn" href="{{route('admin.vehicletype.create')}}">
...
<script type="text/javascript">
	$(document).on("click","a.deletevehiclecategories",function(e){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url:"{{route('admin.vehicletype.destroy',[''])}}"+"/"+id,
					type: 'DELETE',
	...
	$(document).on("click",".changeStatusRecord",function(e){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url:"{{ route('admin.vehicletype.changestatus','replaceid') }}",
					type: 'post',
					data: {"_method": 'post',
					'id':id,
					"_token": "{{ csrf_token() }}"
