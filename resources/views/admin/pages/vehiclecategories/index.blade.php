			...
			<div class="ibox ">
				<div class="ibox-content">
					<div class="col-md-12 text-right">
						@php 
						$checkPermission = \App\Helpers\Helper::checkPermission(['vehicle-create']);
						@endphp
						@if($checkPermission)
						<a class="btn btn-primary btn-sm pull-right mb-3 op-btn" href="{{route('admin.vehiclecreate')}}">
...
<script type="text/javascript">
	$(document).on("click","a.deletevehiclecategories",function(e){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url:"{{route('admin.vehicledelete',[''])}}"+"/"+id,
					type: 'post',
	...
	$(document).on("click",".changeStatusRecord",function(e){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url:"{{ route('admin.vehiclechange_status','replaceid') }}",
					type: 'post',
					data: {"_method": 'post',
					'id':id,
					"_token": "{{ csrf_token() }}"
				...
				swal("Cancelled", "Your Vehicle Status is safe :)", "error");
			}
		});
		return false;
	})
