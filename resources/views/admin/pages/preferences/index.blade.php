			...
			<div class="ibox ">
				<div class="ibox-content">
					<div class="col-md-12 text-right">
						...
						@if(auth()->guard('web')->user()->hasPermissionTo('add-preferences'))
						<a class="btn btn-primary btn-sm pull-right mb-3 op-btn" href="{{route('admin.preference.create')}}">
...
<script type="text/javascript">
	$(document).on("click","a.deleteuser",function(e){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url:"{{route('admin.preference.delete',[''])}}"+"/"+id,
					type: 'post',
	...
	$(document).on("click",".changeStatusRecord",function(e){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url:"{{ route('admin.preference.change_status','replaceid') }}",
					type: 'post',
					data: {"_method": 'post',
					'id':id,
					"_token": "{{ csrf_token() }}"
