...
@section('mainContent')
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-10">
            <h2>Emergency Request Management</h2>
        </div>
    </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on("change",".changerequeststatus",function(e){
            var row = $(this);
            var id = $(this).attr('data-id');
            var status = $(this).val();
            swal({
                title: "Are you sure You want's to update this record status?",
                text: " ",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#e69a2a",
                ...
                if (isConfirm) {
                    $.ajax({
                        url:"{{ route('admin.emergecyrequest_changestatus','replaceid') }}",
                        type: 'post',
                        data: {"_method": 'post',
                            'id':id, status: status,
                            "_token": "{{ csrf_token() }}"
